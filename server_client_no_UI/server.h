...
int fifty_fifty(Question q, int level, int answers[2]) {
    srand(time(0));
    int correct_answer = q.answer[level - 1]; // Ðáp án dúng
    int incorrect_answer;

    // Ch?n ng?u nhiên m?t dáp án sai khác dáp án dúng
    do {
        incorrect_answer = rand() % 4 + 1;  // T?o m?t dáp án sai ng?u nhiên t? 1 d?n 4
    } while (incorrect_answer == correct_answer);

    // Ð?t dáp án dúng vào `answers[0]` và dáp án sai vào `answers[1]`
...
int handle_play_game(Message msg, int conn_fd, Question *questions, int level){
    ...
    case OVER_TIME:
     ...
     case FIFTY_FIFTY:
        // X? lý tr? giúp 50/50
        printf("[%d]: Client yêu c?u tr? giúp 50/50 cho câu h?i %d\n", conn_fd, level);
        int answers[2];
        ...
        break;
    case CALL_PHONE:
      printf("[%d]: Client yêu c?u tr? giúp g?i di?n tho?i cho ngu?i thân câu h?i %d\n", conn_fd, level);
      int phone_answer[1];
    ...
    case CHANGE_QUESTION:
      printf("[%d]: Client yêu c?u tr? giúp d?i câu h?i %d\n", conn_fd, level);
...
void *thread_start(void *client_fd)
{
    ...
    case UN_AUTH:
      ...
      case LOGIN:
        ...
        if (re == LOGIN_SUCCESS)
        {
          ...
          printf("[%d]: Ðang nh?p thành công!\n", conn_fd);
          send(conn_fd, &msg, sizeof(msg), 0);
        }
        else if (re == LOGGED_IN)
        {
          msg.type = LOGGED_IN;
          printf("[%d] Tài kho?n dã du?c dang nh?p\n", conn_fd);
          send(conn_fd, &msg, sizeof(msg), 0);
        }
        else if (re == ACCOUNT_BLOCKED)
        {
          msg.type = ACCOUNT_BLOCKED;
          printf("[%d] Tài kho?n dã b? khóa\n", conn_fd);
          send(conn_fd, &msg, sizeof(msg), 0);
        }
        else if (re == ACCOUNT_NOT_EXIST)
        {
          msg.type = ACCOUNT_NOT_EXIST;
          printf("[%d] Tài kho?n không t?n t?i\n", conn_fd);
          send(conn_fd, &msg, sizeof(msg), 0);
        }
      ...
      case SIGNUP:
        {
          ...
          if (re == SIGNUP_SUCCESS)
          {
            ...
            printf("[%d]: Ðang ký thành công!\n", conn_fd);
            send(conn_fd, &msg, sizeof(msg), 0);
          }
          else if (re == ACCOUNT_EXIST)
          {
            msg.type = ACCOUNT_EXIST;
            printf("[%d] Tài kho?n dã t?n t?i\n", conn_fd);
            send(conn_fd, &msg, sizeof(msg), 0);
          }
        }
      }
  ...
  if (recvBytes <= 0)
  {
    printf("[%d]: Client dã ng?t k?t n?i!\n", conn_fd);
    close(conn_fd);
